...
interface MazeExplorer3DProps {
  level: string;
  onComplete: (correct: number, total: number) => void;
}

export const MazeExplorer3D: React.FC<MazeExplorer3DProps> = ({ level, onComplete }) => {
  ...
  useEffect(() => {
    ...
    maze.forEach((row, i) => {
      row.forEach((cell, j) => {
        ...
        if (i === start[0] && j === start[1]) color = 0x22c55e;
        else if (i === end[0] && j === end[1]) color = 0xef4444;
        else if (cell === 1) color = 0x334155;
        else if (visited.has(`${i},${j}`)) color = 0x3b82f6;
  ...
  const dfs = async () => {
    ...
    const search = async (i: number, j: number): Promise<boolean> => {
      if (i < 0 || i >= size || j < 0 || j >= size || maze[i][j] === 1 || visitedSet.has(`${i},${j}`)) {
        return false;
      }
      ...
      if (await search(i - 1, j)) return true;
      if (await search(i + 1, j)) return true;
      if (await search(i, j - 1)) return true;
      if (await search(i, j + 1)) return true;
    ...
    const found = await search(start[0], start[1]);
    setSearching(false);
    
    if (found) {
      toast.success('Path found with DFS! 🎉');
      setCompleted(true);
    ...
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span>🧭 Maze Explorer: Depth Dive</span>
          <span className="text-sm text-muted-foreground">DFS Pathfinding</span>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm">
            ...
            Watch it dive deep into the maze!
          </p>
          <div className="flex gap-2">
            <Button onClick={dfs} disabled={searching || completed}>
